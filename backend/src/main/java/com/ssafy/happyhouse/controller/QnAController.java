...
public class QnAController {
    ...
    @ApiOperation(value = "qnaNo에 해당하는 글의 정보를 반환한다.", response = List.class)
	@GetMapping("{qnaNo}")
	public ResponseEntity<QnA> detailQnA(@PathVariable int qnaNo) throws Exception {
		logger.debug("detailQnA - 호출");
		return new ResponseEntity<QnA>(qnaService.detailQnA(qnaNo), HttpStatus.OK);
	}
    
    @ApiOperation(value = "새로운 QnA 게시글 정보를 입력한다. 그리고 DB입력 성공여부에 따라 'success' 또는 'fail' 문자열을 반환한다.", response = String.class)
	@PostMapping
    ...
    @ApiOperation(value = "qnaNo에 해당하는 게시글의 정보를 수정한다. 그리고 DB수정 성공여부에 따라 'success' 또는 'fail' 문자열을 반환한다.", response = String.class)
	@PutMapping("{qnaNo}")
    ...
    @ApiOperation(value = "qnaNo에 해당하는 게시글의 정보를 삭제한다. 그리고 DB삭제 성공여부에 따라 'success' 또는 'fail' 문자열을 반환한다.", response = String.class)
	@DeleteMapping("{qnaNo}")
